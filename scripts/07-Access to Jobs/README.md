...
The raw outputs (`auto_jobs_access.csv` and `transit_jobs_access.csv`) are saved to the model project folder (N:\rtp_2026_2050\final_runs\sc_base_year_2023_final\soundcast\outputs\access). To avoid unintentional edits to the model output data, we copy/paste the CSV files to the network displaement risk project folder within the indicator\data subfolder. 
