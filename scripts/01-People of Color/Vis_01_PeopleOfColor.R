...
# 2016 data
race_2016 <- read.csv("../Displacement_Risk_Script/data/001_peopleOfColor.csv")
race_2016 <- race_2016 %>% rename(prop_notwhite = "per_notwhite")
race_2016$per_notwhite <- race_2016$prop_notwhite * 100

# 2019 data
race_2019 <- read.csv("../Displacement Index 2021/data/01-People of Color/01_peopleOfColor.csv")
...
mean_race_2019 = mean(race_2019$per_notwhite,na.rm = TRUE)
mean_race_2024 = mean(race$per_notwhite,na.rm = TRUE)


# Bind data sets
race_2016 <- race_2016 %>% select(GEOID, starts_with("est"), per_notwhite)
race_2016$year = as.factor(2016)
race_2019$year = as.factor(2019)
race$year = as.factor(2024)

race_all <- rbind(race_2016, race_2019, race)
...
race_all %>% ggplot(aes(per_notwhite, fill = year))+
  ...
  geom_vline(aes(xintercept=mean_race_2024),
