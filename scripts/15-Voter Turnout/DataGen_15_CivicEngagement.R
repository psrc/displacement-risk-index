...
# Voter turnout is an indicator that is also used in the Equity Tracker project. Because of the overlap, the data set has been developed within the Equity Tracker GitHub repo (GitHub\equity-tracker\data_development\voter_turnout). This folder contains all of the associated documentation and scripts. The 3-voter-turnout-12-16-20-24.qmd file documents the development process and creates the final data set that is used in the Equity Tracker (voter turnout data for presidential election years, 2012-2024) - the same one that will be used in this script. This script must be re-run after any updates to the 1) ACS 5y data (impacts tract-based equity quintiles and the total population values used to weight - the ratio of adult 18+/total population estimates); 2) base year (adjusts the precinct-tract splits based on parcels total population values).
# For the Displacement Risk Index project, we will be focused on the most recent year - for this update it is 2024. The input data set for both projects is saved within the Equity Tracker network project folder (Y:/Equity Indicators/tracker-webpage-content/h-public-services/h02-voter-participation/raw-data/voter_participation.csv). This file is organized by census tracts (2010 or 2020, depending on the election year). It was generated by creating voting precinct-census tract geography splits and using OFM SAEP and ACS 5y population estimates. More information about the rational and calculations is available in the 3-voter-turnout-12-16-20-24.qmd file. 

# Accessing voter turn out data --------------------------------------
...
# for the displacement risk index we will focus on most recent election year
election_recent <- all_election %>% 
  filter(election_year==2024)

# explore filtered data set - should only be one year of census tracts
str(election_recent)

## Transforming data ----
# configuring data set for viz script: csv (15_CivicEngagement.csv) with GEOID, votes fields
# generating data set to match 2021 update
election_simplified <- election_recent %>% 
  select(planning_geog, pop18_turnout) %>% 
  rename(GEOID=planning_geog,
         votes=pop18_turnout) %>% 
  mutate(votes=round(votes*100, digits=8))

str(election_simplified)
...
tract <- merge(tract, election_simplified,
               by.x="geoid20",
               by.y="GEOID",
               all.x=TRUE)

tract$votes <- round(tract$votes, digits = 2)


# Exporting data sets ----------------------------------------------- 

## Final data set for percentage by census tract ----
write_csv(election_simplified, "./15_CivicEngagement.csv")
...
# rds_2021 <- readRDS("Y:/VISION 2050/Data/Displacement/Displacement Index 2021/data/15-Voter Turnout/tract_15_CivicEngagement.rds")
...
loaded_data <- readRDS("./tract_15_CivicEngagement.rds")
